...
INT frontend_init() {  // I think I have updated this for the uac
  ...
  if ( status != DB_SUCCESS && status != DB_OPEN_RECORD ) 
  {
    cm_msg( MERROR, "frontend_init",
               " Failed to create record \"/Elog/Elog Parameters\": %i, EXITING ",
	    status );
    sleep(2);
    return 0;
  }
  db_find_key( hDB, 0, "/Elog/Elog Parameters/", &hKey );
  if (db_open_record( hDB, hKey, &elog_param, sizeof(elog_param),
                      MODE_READ, 0, 0) 
       != DB_SUCCESS) 
  {
    cm_msg( MERROR, "frontend_init",
            "cannot open \"/Elog/Elog Parameters\" tree in ODB");
    return 0;
  }
...
INT begin_of_run( INT run_number, char *error ) {  // I think this is updated for uac
  ...
  entry<<"elog"
       <<" -h "<<elog_param.hostname
       <<" -p "<<elog_param.port
       <<" -l "<<elog_param.logbook_name
       <<" -a Subject=\"Run Start\""
       <<" -a \"Acquisition Mode\"=MIDAS"
       <<" -a Experiment="<<elog_param.Experiment
       <<" -a Sample="<<elog_param.Sample
       <<" -a Type=\"Auto Entry\""
       <<" -a author=daq"
       <<" -a \"Run Number\"="<<setw(6)<< setfill('0') << runinfo.run_number
       <<" -a Detectors=\" ";
  for(int a=0; a<arrsize; a++) {
    if(a<arrsize-1) {
      entry<<detname[a]<<" | ";
  ...
  entry<<" \" \"Auto ELog Start of Run\" -u "<<elog_param.elog_user<<" "<<elog_param.elog_passwd;
...
INT end_of_run( INT run_number, char *error ) {  // Updated for uac--but still Logger issue
 ...
 ifstream vconfig;
  ...
  entry<<" \" \""
       <<message.str()<<" \" -u "<<elog_param.elog_user<<" "<<elog_param.elog_passwd;
